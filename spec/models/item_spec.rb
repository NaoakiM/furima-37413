...
RSpec.describe Item, type: :model do
  ...
  context '商品登録ができない時' do
    ...
    it '商品のカテゴリーの情報が空だと保存できない' do
      ...
      @item.valid?
      expect(@item.errors.full_messages).to include("Item categoryは1以外の値にしてください")
    end

    it '商品の状態の情報が空だと保存できない' do
      @item.item_sales_status_id = '1'
      @item.valid?
      expect(@item.errors.full_messages).to include("Item sales statusは1以外の値にしてください")
    end

    it '配送料の負担の情報が空だと保存できないこと' do
      @item.item_shipping_fee_status_id = '1'
      @item.valid?
      expect(@item.errors.full_messages).to include("Item shipping fee statusは1以外の値にしてください")
    end

    it '発送元の地域の情報が空だと保存できないこと' do
      @item.item_prefecture_id = '1'
      @item.valid?
      expect(@item.errors.full_messages).to include("Item prefectureは1以外の値にしてください")
    end

    it '発送までの日数の情報が空だと保存できないこと' do
      @item.item_scheduled_delivery_id = '1'
      @item.valid?
      expect(@item.errors.full_messages).to include("Item scheduled deliveryは1以外の値にしてください")
    end
